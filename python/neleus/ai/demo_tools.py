...
def get_available_markets() -> List[str]:
    ...
    try:
        ...
        logger.info(
            f"Loaded {len(_AVAILABLE_MARKETS)} markets from Hyperliquid")
            ...
            try:
                ...
                if candles and len(candles) >= 10:
                    ...
                    historical_vol = np.std(
                        returns[:-6]) * np.sqrt(24 * 365) * 100
                    vol_change = ((recent_vol - historical_vol) /
                                  historical_vol) * 100 if historical_vol > 0 else 0
                    ...
                    if include_forecast:
                        ...
                        for r in returns[-12:-1]:
                            ewma_var = lambda_param * ewma_var + \
                                (1 - lambda_param) * r ** 2
                        forecast_vol = np.sqrt(
                            ewma_var) * np.sqrt(24 * 365) * 100
                    ...
                    output["recommendations"] = self._get_recommendations(
                        regime, risk_level)
                ...
                logger.warning(
                    f"Real data fetch failed, using simulation: {e}")
    ...
    async def execute(
        ...
        try:
            ...
            for instrument in instruments:
                ...
                try:
                    ...
                    if candles and len(candles) >= 20:
                        ...
                        trend = "bullish" if sma_short > sma_long * \
                            1.01 else "bearish" if sma_short < sma_long * 0.99 else "neutral"

                        # Momentum
                        roc = (closes[-1] - closes[-24]) / closes[-24] * 100
                        momentum = "strong" if abs(
                            roc) > 5 else "moderate" if abs(roc) > 2 else "weak"

                        # Volume analysis
                        vol_sma = np.mean(volumes[-168:])
                        vol_recent = np.mean(volumes[-24:])
                        volume_regime = "high" if vol_recent > vol_sma * \
                            1.5 else "low" if vol_recent < vol_sma * 0.7 else "normal"
            ...
            bullish_count = sum(1 for r in results.values()
                                if r.get("trend") == "bullish")
            bearish_count = sum(1 for r in results.values()
                                if r.get("trend") == "bearish")
            ...
            try:
                ...
                if candles and len(candles) >= 100:
                    ...
                    var_pct = np.percentile(
                        returns, (1 - confidence_level) * 100)
                    ...
                    recommended_size = (
                        max_risk_pct / (abs(var_pct) * 100 + 0.01)) * position_size
